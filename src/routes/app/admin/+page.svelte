<script lang="ts">
</script>

<div class="min-h-screen bg-gray-50 px-4 py-8 sm:px-6 lg:px-8">
	<div class="mx-auto max-w-4xl">
		<h1 class="text-3xl font-bold tracking-tight text-gray-900">Admin</h1>
		<p class="mt-2 text-sm text-gray-600">Manage your organization.</p>

		<div class="mt-8 grid gap-4 sm:grid-cols-2">
			<a
				href="/app/admin/201"
				class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition hover:border-primary/50 hover:shadow-md"
			>
				<h2 class="text-lg font-semibold text-gray-900">201 Files</h2>
				<p class="mt-1 text-sm text-gray-600">Review and approve employee 201 profiles.</p>
			</a>
			<a
				href="/app/admin/employees/invite"
				class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition hover:border-primary/50 hover:shadow-md"
			>
				<h2 class="text-lg font-semibold text-gray-900">Invite employee</h2>
				<p class="mt-1 text-sm text-gray-600">Send an invitation to join your organization.</p>
			</a>
		</div>
	</div>
</div>